...
export default function ResultsPage() {
  ...
  return (
    <main style={{ backgroundColor: '#222222', minHeight: '100vh' }} className="flex flex-col items-center p-8 overflow-hidden relative">
      ...
      {/* AI Summary */}
      {aiSummary && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="w-full max-w-2xl mb-6 relative z-10">
          <div className="rounded-xl p-5" style={{ backgroundColor: '#2a2a2a', border: '1px solid #89E90033' }}>
            <p className="text-sm font-semibold mb-3" style={{ color: '#89E900', letterSpacing: 1 }}>✦ AI SUMMARY</p>
            <p style={{ color: '#ffffff80', fontSize: 14, lineHeight: 1.8 }}>{aiSummary}</p>
          </div>
        </motion.div>
      )}
            ...
            >
              {!emailSubmitted ? (
                <>
                  ...
                  <motion.button onClick={handleEmailSubmit} disabled={submitting || !email} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                ...
                <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center">
                  ...
                  <motion.button onClick={() => setShowEmailModal(false)} whileHover={{ scale: 1.02 }}
